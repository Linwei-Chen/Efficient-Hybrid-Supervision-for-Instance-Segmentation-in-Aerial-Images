...
def lovasz_grad(gt_sorted):
    ...
    #############
    # print(gt_sorted.type())
    # union = gts.float() + (1 - gt_sorted).float().cumsum(0)
    union = gts.float() + (1 - gt_sorted.int()).float().cumsum(0)
...
if __name__ == '__main__':
    ...
    for i in range(-200, 200, 1):
        x = 0.01 * i
        # loss = lovasz_hinge(torch.tensor([[x], [x]]), torch.tensor([0, 1]), ignore=255).item()
        loss = lovasz_hinge(torch.rand(3, 2, 4, 4), torch.randint(0, 2, (3, 2, 4, 4)), ignore=255).item()
        loss_list.append([x, loss])
    plt.scatter([i[0] for i in loss_list], [i[1] for i in loss_list], alpha=0.6)
    plt.draw()
    b = torch.tensor([1, 1, 2]).byte()
    print(b.float())
    pass
